...
func TestDocumentCreate(t *testing.T) {
	...
	result, err := typosearchClient.Collection(collectionName).Documents().Create(context.Background(), document, &api.DocumentIndexParameters{})
...
func TestDocumentUpsertNewDocument(t *testing.T) {
	...
	document := newDocument("123")
	result, err := typosearchClient.Collection(collectionName).Documents().Upsert(context.Background(), document, &api.DocumentIndexParameters{})
...
func TestDocumentUpsertExistingDocument(t *testing.T) {
	...
	document := newDocument("123")
	_, err := typosearchClient.Collection(collectionName).Documents().Create(context.Background(), document, &api.DocumentIndexParameters{})
	require.NoError(t, err)
...
func TestDocumentsDelete(t *testing.T) {
	...
	result, err := typosearchClient.Collection(collectionName).Documents().Delete(context.Background(), filter)
	...
	require.NoError(t, err)
	_, err = typosearchClient.Collection(collectionName).Document("124").Retrieve(context.Background())
	require.Error(t, err)
}
...
func TestDocumentsExport(t *testing.T) {
	...
	body, err := typosearchClient.Collection(collectionName).Documents().Export(context.Background(), &api.ExportDocumentsParams{})
	require.NoError(t, err)
	defer body.Close()
