...
func TestSearchSynonymUpsertNewSynonym(t *testing.T) {
	...
	result, err := typosearchClient.Collection(collectionName).Synonyms().Upsert(context.Background(), synonymID, body)
...
func TestSearchSynonymsRetrieve(t *testing.T) {
	...
	for i := 0; i < total; i++ {
		_, err := typosearchClient.Collection(collectionName).Synonyms().Upsert(context.Background(), synonymIDs[i], schema)
		require.NoError(t, err)
	}

	result, err := typosearchClient.Collection(collectionName).Synonyms().Retrieve(context.Background())
