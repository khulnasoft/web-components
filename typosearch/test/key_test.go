...
func TestKeyRetrieve(t *testing.T) {
	...
	result, err := typosearchClient.Key(*expectedKey.Id).Retrieve(context.Background())
...
func TestKeyDelete(t *testing.T) {
	...
	result, err := typosearchClient.Key(*expectedKey.Id).Delete(context.Background())

	require.NoError(t, err)
	require.Equal(t, expectedKey.Id, result.Id)

	_, err = typosearchClient.Key(*expectedKey.Id).Retrieve(context.Background())
	require.Error(t, err)
}
