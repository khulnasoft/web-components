...
func presetsCleanUp() {
	result, _ := typosearchClient.Presets().Retrieve(context.Background())
	for _, preset := range result {
		typosearchClient.Preset(preset.Name).Delete(context.Background())
	}
}
...
func TestPresetsUpsertValueFromSearchParameters(t *testing.T) {
	...
	result, err := typosearchClient.Presets().Upsert(context.Background(), presetID, body)
...
func TestPresetsRetrievePresetFromSearchParameters(t *testing.T) {
	...
	for i := 0; i < total; i++ {
		_, err := typosearchClient.Presets().Upsert(context.Background(), presetIDs[i], schema)
		require.NoError(t, err)
	}

	result, err := typosearchClient.Presets().Retrieve(context.Background())
