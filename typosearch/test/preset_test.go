...
func TestPresetValueFromSearchParametersRetrieve(t *testing.T) {
	...
	result, err := typosearchClient.Preset(presetName).Retrieve(context.Background())
...
func TestPresetDelete(t *testing.T) {
	...
	result, err := typosearchClient.Preset(presetName).Delete(context.Background())

	require.NoError(t, err)
	require.Equal(t, expectedResult.Name, result.Name)

	_, err = typosearchClient.Preset(presetName).Retrieve(context.Background())
	require.Error(t, err)
}
