...
func TestSearchOverrideUpsertNewOverride(t *testing.T) {
	...
	result, err := typosearchClient.Collection(collectionName).Overrides().Upsert(context.Background(), overrideID, body)
...
func TestSearchOverridesRetrieve(t *testing.T) {
	...
	for i := 0; i < total; i++ {
		_, err := typosearchClient.Collection(collectionName).Overrides().Upsert(context.Background(), overrideIDs[i], schema)
		require.NoError(t, err)
	}

	result, err := typosearchClient.Collection(collectionName).Overrides().Retrieve(context.Background())
