...
func TestStopwordsRetrieve(t *testing.T) {
	...
	for i := 0; i < total; i++ {
		_, err := typosearchClient.Stopwords().Upsert(context.Background(), stopwordsSetIDs[i], upsertData)
		require.NoError(t, err)
	}

	result, err := typosearchClient.Stopwords().Retrieve(context.Background())
