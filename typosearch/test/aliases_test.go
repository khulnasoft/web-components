...
func TestCollectionAliasUpsertNewAlias(t *testing.T) {
	...
	result, err := typosearchClient.Aliases().Upsert(context.Background(), aliasName, body)
...
func TestCollectionAliasesRetrieve(t *testing.T) {
	...
	for i := 0; i < total; i++ {
		_, err := typosearchClient.Aliases().Upsert(context.Background(), aliasNames[i], body)
		require.NoError(t, err)
	}

	result, err := typosearchClient.Aliases().Retrieve(context.Background())
