...
func createNewKey(t *testing.T) *api.ApiKey {
	...
	result, err := typosearchClient.Keys().Create(context.Background(), keySchema)
...
func createNewPreset(t *testing.T, presetValueIsFromSearchParameters ...bool) (string, *api.PresetSchema) {
	...
	result, err := typosearchClient.Presets().Upsert(context.Background(), presetName, presetSchema)
...
func retrieveDocuments(t *testing.T, collectionName string, docIDs ...string) []map[string]interface{} {
	...
	for i, docID := range docIDs {
		doc, err := typosearchClient.Collection(collectionName).Document(docID).Retrieve(context.Background())
		require.NoError(t, err)
		results[i] = doc
	}
	return results
