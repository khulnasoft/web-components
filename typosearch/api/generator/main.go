...
// https://github.com/khulnasoft/typosearch-api-spec/blob/master/openapi.yml

func main() {
	m := make(yml)

	log.Println("Fetching openapi.yml from typosearch api spec")
	err := fetchOpenAPISpec()
	if err != nil {
		log.Fatalf("Aboring: %s", err.Error())
	}

	configFile, err := os.Open("./typosearch/api/generator/openapi.yml")
	if err != nil {
	...
	generatorFile, err := os.Create("./typosearch/api/generator/generator.yml")
	if err != nil {
		log.Fatalf("Unable to open config file: %s", err.Error())
		return
	}
...
func fetchOpenAPISpec() error {
	url := "https://raw.githubusercontent.com/typosearch/typosearch-api-spec/master/openapi.yml"
	...
	openapiFile, err := os.Create("./typosearch/api/generator/openapi.yml")
	if err != nil {
