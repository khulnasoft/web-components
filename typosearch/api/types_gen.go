...
type CollectionResponse struct {
	...
	// EnableNestedFields Enables experimental support at a collection level for nested object or object array fields. This field is only available if the Typosearch server is version `0.24.0.rcn34` or later.
...
type ConversationModelCreateSchema struct {
	...
	// HistoryCollection Typosearch collection that stores the historical conversations
...
type MultiSearchCollectionParameters struct {
	...
	// ConversationId The Id of a previous conversation to continue, this tells Typosearch to include prior context when communicating with the LLM.
	...
	// DropTokensThreshold If the number of results found for a specific query is less than this number, Typosearch will attempt to drop the tokens in the query until enough results are found. Tokens that have the least individual hits are dropped first. Set to 0 to disable. Default: 10
	...
	// EnableTyposForNumericalTokens Make Typosearch disable typos for numerical tokens.
	...
	// ExhaustiveSearch Setting this to true will make Typosearch consider all prefixes and typo corrections of the words in the query without stopping early when enough results are found (drop_tokens_threshold and typo_tokens_threshold configurations are ignored).
	...
	// PreSegmentedQuery You can index content from any logographic language into Typosearch if you are able to segment / split the text into space-separated words yourself before indexing and querying.
	...
	// PrioritizeNumMatchingFields Make Typosearch prioritize documents where the query words appear in more number of fields.
	PrioritizeNumMatchingFields *bool `json:"prioritize_num_matching_fields,omitempty"`

	// PrioritizeTokenPosition Make Typosearch prioritize documents where the query words appear earlier in the text.
	...
	// SearchCutoffMs Typosearch will attempt to return results early if the cutoff time has elapsed. This is not a strict guarantee and facet computation is not bound by this parameter.
	...
	// TypoTokensThreshold If the number of results found for a specific query is less than this number, Typosearch will attempt to look for tokens with more typos until enough results are found. Default: 100
...
type SearchParameters struct {
	...
	// MaxCandidates Control the number of words that Typosearch considers for typo and prefix searching.
