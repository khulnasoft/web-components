...
protected:
    ...
    void setupCollection() {
        std::string state_dir_path = "/tmp/typosearch_test/coll_manager_test_db";
...
TEST_F(CollectionManagerTest, RestoreRecordsOnRestart) {
    ...
    store = new Store("/tmp/typosearch_test/coll_manager_test_db");
    collectionManager.init(store, 1.0, "auth_key", quit);
...
TEST_F(CollectionManagerTest, Symlinking) {
    ...
    std::string state_dir_path = "/tmp/typosearch_test/cmanager_test_db";
    system(("rm -rf "+state_dir_path+" && mkdir -p "+state_dir_path).c_str());
