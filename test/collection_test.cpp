...
protected:
    ...
    void setupCollection() {
        std::string state_dir_path = "/tmp/typosearch_test/collection";
        LOG(INFO) << "Truncating and creating: " << state_dir_path;
        system(("rm -rf "+state_dir_path+" && mkdir -p "+state_dir_path).c_str());
        system("mkdir -p /tmp/typosearch_test/models");
...
TEST_F(CollectionTest, DISABLED_SearchingForRecordsWithSpecialChars) {
    ...
    results = coll1->search("typosearch",
                            {"title", "url"}, "", {}, {}, {2}, 10, 1, FREQUENCY).get();
...
TEST_F(CollectionTest, SemanticSearchTest) {
    ...
    EmbedderManager::set_model_dir("/tmp/typosearch_test/models");
...
TEST_F(CollectionTest, CreateModelDirIfNotExists) {
    system("mkdir -p /tmp/typosearch_test/new_models_dir");
    system("rm -rf /tmp/typosearch_test/new_models_dir");
    EmbedderManager::set_model_dir("/tmp/typosearch_test/new_models_dir");

    // check if model dir is created
    ASSERT_TRUE(std::filesystem::exists("/tmp/typosearch_test/new_models_dir"));
}
