...
#include "typosearch_server_utils.h"
...
void catch_interrupt(int sig) {
    LOG(INFO) << "Stopping Typosearch server...";
...
void init_cmdline_options(cmdline::parser & options, int argc, char **argv) {
    options.set_program_name("./typosearch-server");
    ...
    options.add<std::string>("api-address", '\0', "Address to which Typosearch API service binds.", false, "0.0.0.0");
    options.add<uint32_t>("api-port", '\0', "Port on which Typosearch API service listens.", false, 8108);

    options.add<std::string>("peering-address", '\0', "Internal IP address to which Typosearch peering service binds.", false, "");
    options.add<uint32_t>("peering-port", '\0', "Port on which Typosearch peering service listens.", false, 8107);
    options.add<std::string>("peering-subnet", '\0', "Internal subnet that Typosearch should use for peering.", false, "");
    ...
    // DEPRECATED
    options.add<std::string>("listen-address", 'h', "[DEPRECATED: use `api-address`] Address to which Typosearch API service binds.", false, "0.0.0.0");
    options.add<uint32_t>("listen-port", 'p', "[DEPRECATED: use `api-port`] Port on which Typosearch API service listens.", false, 8108);
...
int init_root_logger(Config & config, const std::string & server_version) {
    google::InitGoogleLogging("typosearch");
    ...
    if(log_dir.empty()) {
        ...
        if(!directory_exists(log_dir)) {
            std::cerr << "Typosearch failed to start. " << "Log directory " << log_dir << " does not exist.";
            return 1;
        }
        ...
        std::string log_path = log_dir + "/" + "typosearch.log";
...
int start_raft_server(ReplicationState& replication_state, Store& store,
    ...
    if(path_to_nodes.empty()) {
        LOG(INFO) << "Since no --nodes argument is provided, starting a single node Typosearch cluster.";
    }
    ...
    LOG(INFO) << "Typosearch peering service is running on " << raft_server.listen_address();
    ...
    LOG(INFO) << "Typosearch peering service is going to quit.";
    ...
    LOG(INFO) << "Typosearch peering service has quit.";

    return 0;
}

int run_server(const Config & config, const std::string & version, void (*master_server_routes)()) {
    LOG(INFO) << "Starting Typosearch " << version << std::flush;
#ifndef ASAN_BUILD
    if(using_jemalloc()) {
        LOG(INFO) << "Typosearch is using jemalloc.";
        ...
        LOG(WARNING) << "Typosearch is NOT using jemalloc.";
    }
#endif
    ...
    if(!directory_exists(config.get_data_dir())) {
        LOG(ERROR) << "Typosearch failed to start. " << "Data directory " << config.get_data_dir()
                 << " does not exist.";
        return 1;
    }
    ...
    LOG(INFO) << "Typosearch API service has quit.";
