...
void Config::load_config_env() {
    this->data_dir = get_env("TYPOSEARCH_DATA_DIR");
    this->log_dir = get_env("TYPOSEARCH_LOG_DIR");
    this->api_key = get_env("TYPOSEARCH_API_KEY");

    // @deprecated
    this->search_only_api_key = get_env("TYPOSEARCH_SEARCH_ONLY_API_KEY");

    this->health_rusage_api_key = get_env("TYPOSEARCH_HEALTH_RUSAGE_API_KEY");

    if(!get_env("TYPOSEARCH_LISTEN_ADDRESS").empty()) {
        this->api_address = get_env("TYPOSEARCH_LISTEN_ADDRESS");
    }

    if(!get_env("TYPOSEARCH_LISTEN_PORT").empty()) {
        this->api_port = std::stoi(get_env("TYPOSEARCH_LISTEN_PORT"));
    }

    if(!get_env("TYPOSEARCH_API_ADDRESS").empty()) {
        this->api_address = get_env("TYPOSEARCH_API_ADDRESS");
    }

    if(!get_env("TYPOSEARCH_API_PORT").empty()) {
        this->api_port = std::stoi(get_env("TYPOSEARCH_API_PORT"));
    }

    if(!get_env("TYPOSEARCH_PEERING_ADDRESS").empty()) {
        this->peering_address = get_env("TYPOSEARCH_PEERING_ADDRESS");
    }

    if(!get_env("TYPOSEARCH_PEERING_PORT").empty()) {
        this->peering_port = std::stoi(get_env("TYPOSEARCH_PEERING_PORT"));
    }

    if(!get_env("TYPOSEARCH_PEERING_SUBNET").empty()) {
        this->peering_subnet = get_env("TYPOSEARCH_PEERING_SUBNET");
    }

    this->nodes = get_env("TYPOSEARCH_NODES");

    this->master = get_env("TYPOSEARCH_MASTER");
    this->ssl_certificate = get_env("TYPOSEARCH_SSL_CERTIFICATE");
    this->ssl_certificate_key = get_env("TYPOSEARCH_SSL_CERTIFICATE_KEY");

    std::string enable_cors_str = get_env("TYPOSEARCH_ENABLE_CORS");
    StringUtils::toupper(enable_cors_str);
    this->enable_cors = ("TRUE" == enable_cors_str || enable_cors_str.empty()) ? true : false;

    std::string cors_domains_value = get_env("TYPOSEARCH_CORS_DOMAINS");
    set_cors_domains(cors_domains_value);

    if(!get_env("TYPOSEARCH_MAX_MEMORY_RATIO").empty()) {
        this->max_memory_ratio = std::stof(get_env("TYPOSEARCH_MAX_MEMORY_RATIO"));
    }

    if(!get_env("TYPOSEARCH_SNAPSHOT_INTERVAL_SECONDS").empty()) {
        this->snapshot_interval_seconds = std::stoi(get_env("TYPOSEARCH_SNAPSHOT_INTERVAL_SECONDS"));
    }

    if(!get_env("TYPOSEARCH_HEALTHY_READ_LAG").empty()) {
        this->healthy_read_lag = std::stoi(get_env("TYPOSEARCH_HEALTHY_READ_LAG"));
    }

    if(!get_env("TYPOSEARCH_HEALTHY_WRITE_LAG").empty()) {
        this->healthy_write_lag = std::stoi(get_env("TYPOSEARCH_HEALTHY_WRITE_LAG"));
    }

    if(!get_env("TYPOSEARCH_LOG_SLOW_REQUESTS_TIME_MS").empty()) {
        this->log_slow_requests_time_ms = std::stoi(get_env("TYPOSEARCH_LOG_SLOW_REQUESTS_TIME_MS"));
    }

    if(!get_env("TYPOSEARCH_LOG_SLOW_SEARCHES_TIME_MS").empty()) {
        this->log_slow_searches_time_ms = std::stoi(get_env("TYPOSEARCH_LOG_SLOW_SEARCHES_TIME_MS"));
    }

    if(!get_env("TYPOSEARCH_NUM_COLLECTIONS_PARALLEL_LOAD").empty()) {
        this->num_collections_parallel_load = std::stoi(get_env("TYPOSEARCH_NUM_COLLECTIONS_PARALLEL_LOAD"));
    }

    if(!get_env("TYPOSEARCH_NUM_DOCUMENTS_PARALLEL_LOAD").empty()) {
        this->num_documents_parallel_load = std::stoi(get_env("TYPOSEARCH_NUM_DOCUMENTS_PARALLEL_LOAD"));
    }

    if(!get_env("TYPOSEARCH_CACHE_NUM_ENTRIES").empty()) {
        this->cache_num_entries = std::stoi(get_env("TYPOSEARCH_CACHE_NUM_ENTRIES"));
    }

    if(!get_env("TYPOSEARCH_ANALYTICS_FLUSH_INTERVAL").empty()) {
        this->analytics_flush_interval = std::stoi(get_env("TYPOSEARCH_ANALYTICS_FLUSH_INTERVAL"));
    }

    if(!get_env("TYPOSEARCH_HOUSEKEEPING_INTERVAL").empty()) {
        this->housekeeping_interval = std::stoi(get_env("TYPOSEARCH_HOUSEKEEPING_INTERVAL"));
    }

    if(!get_env("TYPOSEARCH_DB_COMPACTION_INTERVAL").empty()) {
        this->db_compaction_interval = std::stoi(get_env("TYPOSEARCH_DB_COMPACTION_INTERVAL"));
    }

    if(!get_env("TYPOSEARCH_THREAD_POOL_SIZE").empty()) {
        this->thread_pool_size = std::stoi(get_env("TYPOSEARCH_THREAD_POOL_SIZE"));
    }

    if(!get_env("TYPOSEARCH_SSL_REFRESH_INTERVAL_SECONDS").empty()) {
        this->ssl_refresh_interval_seconds = std::stoi(get_env("TYPOSEARCH_SSL_REFRESH_INTERVAL_SECONDS"));
    }

    if(!get_env("TYPOSEARCH_SNAPSHOT_MAX_BYTE_COUNT_PER_RPC").empty()) {
        this->snapshot_max_byte_count_per_rpc = std::stoi(get_env("TYPOSEARCH_SNAPSHOT_MAX_BYTE_COUNT_PER_RPC"));
    }

    this->enable_access_logging = ("TRUE" == get_env("TYPOSEARCH_ENABLE_ACCESS_LOGGING"));
    this->enable_search_analytics = ("TRUE" == get_env("TYPOSEARCH_ENABLE_SEARCH_ANALYTICS"));
    this->enable_search_logging = ("TRUE" == get_env("TYPOSEARCH_ENABLE_SEARCH_LOGGING"));

    if(!get_env("TYPOSEARCH_DISK_USED_MAX_PERCENTAGE").empty()) {
        this->disk_used_max_percentage = std::stoi(get_env("TYPOSEARCH_DISK_USED_MAX_PERCENTAGE"));
    }

    if(!get_env("TYPOSEARCH_MEMORY_USED_MAX_PERCENTAGE").empty()) {
        this->memory_used_max_percentage = std::stoi(get_env("TYPOSEARCH_MEMORY_USED_MAX_PERCENTAGE"));
    }

    if(!get_env("TYPOSEARCH_FILTER_BY_MAX_OPS").empty()) {
        this->filter_by_max_ops = std::stoi(get_env("TYPOSEARCH_FILTER_BY_MAX_OPS"));
    }

    this->skip_writes = ("TRUE" == get_env("TYPOSEARCH_SKIP_WRITES"));
    this->enable_lazy_filter = ("TRUE" == get_env("TYPOSEARCH_ENABLE_LAZY_FILTER"));
    this->reset_peers_on_error = ("TRUE" == get_env("TYPOSEARCH_RESET_PEERS_ON_ERROR"));

    if(!get_env("TYPOSEARCH_MAX_PER_PAGE").empty()) {
        this->max_per_page = std::stoi(get_env("TYPOSEARCH_MAX_PER_PAGE"));
    }

    if(!get_env("TYPOSEARCH_ANALYTICS_DIR").empty()) {
        this->analytics_dir = get_env("TYPOSEARCH_ANALYTICS_DIR");
    }

    if(!get_env("TYPOSEARCH_ANALYTICS_DB_TTL").empty()) {
        this->analytics_db_ttl = std::stoi(get_env("TYPOSEARCH_ANALYTICS_DB_TTL"));
    }

    if(!get_env("TYPOSEARCH_ANALYTICS_MINUTE_RATE_LIMIT").empty()) {
        this->analytics_minute_rate_limit = std::stoi(get_env("TYPOSEARCH_ANALYTICS_MINUTE_RATE_LIMIT"));
    }
}
