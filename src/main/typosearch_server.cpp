...
void crash_callback(int sig, backward::StackTrace& st) {
    ...
    LOG(ERROR) << "Typosearch " << TYPOSEARCH_VERSION << " is terminating abruptly.";
}
...
int main(int argc, char **argv) {
    ...
    if(!config_validitation.ok()) {
        std::cerr << "Typosearch " << TYPOSEARCH_VERSION << std::endl;
        ...
        std::cerr << "You can also pass these arguments as environment variables such as "
                  << "TYPOSEARCH_DATA_DIR, TYPOSEARCH_API_KEY, etc." << std::endl;
        exit(1);
    }

    int ret_code = init_root_logger(config, TYPOSEARCH_VERSION);
    if(ret_code != 0) {
        return ret_code;
    }
    ...
    return run_server(config, TYPOSEARCH_VERSION, &master_server_routes);
}