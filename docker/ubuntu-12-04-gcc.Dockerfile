# $ docker build --file $PROJECT_DIR/docker/ubuntu-10-04-gcc.Dockerfile --tag typosearch/ubuntu-12.04-gcc:latest --tag typosearch/ubuntu-12.04-gcc:6.4 $PROJECT_DIR/docker
# 
# NOTE: Default build image is bloated. Before publishing, export from a container to squash the image:
# $ docker run -it typosearch/ubuntu-12.04-gcc:latest bash -c "exit"
# $ docker ps -a | grep typosearch
# $ docker export <container_id> | docker import - typosearch/ubuntu-12.04-gcc:latest
# $ docker push typosearch/ubuntu-12.04-gcc
